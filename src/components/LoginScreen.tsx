...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!success) setError("Credenciais inválidas. Por favor, tente novamente.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#005058] flex flex-col justify-center items-center p-4">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between mt-6">
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#005058] hover:bg-[#003d44] text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition duration-200 disabled:bg-gray-400 disabled:cursor-not-allowed flex justify-center items-center h-10"
            >
              {loading ? (
                <svg
      ...
      <footer className="text-center text-gray-300 text-xs mt-8">
        <p className="mb-1">Versão 2.1.0</p>
        &copy;{new Date().getFullYear()} Sicoob Cooprem. Todos os direitos reservados.
      </footer>
    </div>
  );
};
